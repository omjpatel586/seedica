...
export default function ProductsPage() {
  ...
  const filteredProducts =
    activeTab === "All"
      ? PRODUCTS_DATA
      : PRODUCTS_DATA.filter((p) => t(`${p.id}.category`) === activeTab);
          ...
          return (
            ...
            >
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <p className="text-gray-500 text-sm mb-4 line-clamp-2">{productDesc}</p>

                <div className="mt-auto pt-4 flex items-center justify-between border-t border-gray-50">
                  <span className="text-2xl font-bold text-emerald-600">{product.price}</span>
                  <Link href={`/products/${product.id}`} locale={locale}>
                    <button className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all cursor-pointer">
                      View Details
                    </button>
                  </Link>
                </div>
      ...
      {filteredProducts.length === 0 && (
        <div className="text-center py-20">
          <p className="text-gray-400 text-lg italic">
            No products found in the {activeTab} category.
          </p>
        </div>
      )}
    </div>
